...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size)
{
    ...
    // HYBRID MOD: 
    // We are NOT summing 4 voices into 1 output. We are sending discrete voices.
    // Therefore, we do not divide by kNumVoices (4). 
    // We only divide by 2 (oscillator count) to prevent clipping when Osc1+Osc2 sum.
    const float mix_scale = 0.3f; 
    ...
    for(size_t i = 0; i < size; ++i)
    {
        ...
        for(int v = 0; v < kNumVoices; ++v)
        {
            ...
            // Apply the corrected scale (0.5f) here
            mix[v] *= mix_scale; 
        }
       
        // MAPPING PRESERVED (Do not touch)
        out[0][i] = mix[0];
        out[1][i] = mix[1]; 
        out[2][i] = mix[3]; // Hardware specific map
        out[3][i] = mix[2]; // Hardware specific map
    }
}
...
void ApplyParameters(uint16_t* vals, int n)
{
    // MAPPING FIX FOR 12-POSITION KNOB:
    // Output range 0.0f - 11.99f handles indices 0 through 11
    ...
    for(int v = 0; v < kNumVoices; ++v)
    {
        ...
        switch(waveIndex1) {
            ...
            // Blank positions 5-11 (Future Expansion)
            case 5:
            case 6:
            case 7:
            case 8:
            ...
            case 10:
            case 11:
                // currently does nothing (keeps previous waveform)
                break; 
        }
